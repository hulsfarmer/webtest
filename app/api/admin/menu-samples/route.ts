...
import {
  readMenuSamples, setMenuVideo, setLogoStyle,
  VIDEO_MENU_KEYS, LOGO_STYLE_IDS, LOGO_DIR,
...
export async function POST(req: NextRequest) {
  ...
  if (ct.includes('multipart/form-data')) {
    ...
    if (!LOGO_STYLE_IDS.includes(style)) return NextResponse.json({ error: '알 수 없는 스타일' }, { status: 400 });
    ...
    // 같은 스타일의 기존 파일 제거(캐시 회피 위해 타임스탬프 파일명)
  ...
  // ── 영상 지정 / 로고 해제 (JSON) ──
  let body: { menuKey?: string; jobId?: string; videoUrl?: string; posterUrl?: string; businessName?: string; clearLogo?: string };
