...
export function PromoTool({ embedded = false, forceMode }: { embedded?: boolean; forceMode?: 'business' | 'event' } = {}) {
  ...
  // YouTube/Instagram 연결 상태 확인 + 콜백(?yt=/?ig=) 처리
  useEffect(() => {
    ...
    else if (ig === 'error') setIgMsg('인스타 연결 실패 — 다시 시도하세요.');
    if (q || ig) window.history.replaceState({}, '', window.location.pathname);
  }, []);
  ...
  async function runPublish() {
    ...
    try {
      if (optDl) {
        ...
        a.href = jobStatus.videoUrl; a.download = `promo_${Date.now()}.mp4`;
        document.body.appendChild(a); a.click(); a.remove();
        setDownloaded(true); done.push('다운로드');
      }
      if (optYt && !ytUrl) {
            ...
            {jobStatus.videoUrl && (
              <div className="glass-card p-4 space-y-3">
                ...
                <label className={`flex items-center gap-2.5 text-sm cursor-pointer ${ytUrl ? 'text-gray-500' : 'text-neutral-200'}`}>
                  ...
                  <span>▶ 유튜브에 올리기 <span className="text-gray-500">(일부공개)</span>{ytUrl && ' — 완료'}</span>
                </label>
                {!ytConnected && (
                ...
                <label className={`flex items-center gap-2.5 text-sm cursor-pointer ${igUrl ? 'text-gray-500' : 'text-neutral-200'}`}>
                  <input type="checkbox" className="w-4 h-4 accent-pink-500" checked={optIg} disabled={!!igUrl} onChange={(e) => setOptIg(e.target.checked)} />
                  <span>📸 인스타 릴스로 올리기{igConnected && igAccount ? ` (@${igAccount})` : ''}{igUrl && ' — 완료'}</span>
                </label>
                {!igConnected && (
                ...
                {pubMsg && <div className="text-xs text-gray-300">{pubMsg}</div>}
                {ytMsg && <div className="text-xs text-gray-300">{ytMsg}</div>}
                {igMsg && <div className="text-xs text-gray-300">{igMsg}</div>}
              </div>
            )}
