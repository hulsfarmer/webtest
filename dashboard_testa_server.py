...
GAPUP_STATE     = os.path.join(os.path.dirname(__file__), "daytrade_gapup_state.json")
GAPUP_DB        = os.path.join(os.path.dirname(__file__), "daytrade_gapup.db")
GAPUP_CHANGELOG = os.path.join(os.path.dirname(__file__), "daytrade_gapup_changelog.json")
...
<div class="bots">
  <div class="bot-card">
    <div class="bot-header">
      ...
      <span><button class="refresh-btn" onclick="loadCoin()">새로고침</button>&nbsp;<a class="bot-link" href="http://138.2.28.57:8081/" >대시보드 ↗</a></span>
    </div>
  ...
  <div class="bot-card">
    <div class="bot-header">
      ...
      <span><button class="refresh-btn" onclick="loadTesta()">새로고침</button>&nbsp;<a class="bot-link" href="http://138.2.12.110:8083/" >대시보드 ↗</a></span>
    </div>
  ...
  <div class="bot-card">
    <div class="bot-header">
      ...
      <span><button class="refresh-btn" onclick="loadEvo()">새로고침</button>&nbsp;<a class="bot-link" href="http://138.2.12.110:8080/" >대시보드 ↗</a></span>
    </div>
    <div id="evo-content"><p style="color:#64748b;font-size:12px;padding:10px">로딩 중...</p></div>
  </div>
  <div class="bot-card">    <div class="bot-header">      <span><span class="status-dot dot-loading" id="dot-swing"></span><span class="bot-title">📊 스윙봇 v5.1 (주식)</span></span>      <span><button class="refresh-btn" onclick="loadSwing()">새로고침</button>&nbsp;<a class="bot-link" href="http://138.2.12.110:8080/swing" >대시보드 ↗</a></span>    </div>    <div id="swing-content"><p style="color:#64748b;font-size:12px;padding:10px">로딩 중...</p></div>  </div>
  <div class="bot-card">
    <div class="bot-header">
      <span><span class="status-dot dot-loading" id="dot-daytrade"></span><span class="bot-title">단타 가상거래봇 ⚡</span></span>
      <span><button class="refresh-btn" onclick="loadDaytrade()">새로고침</button>&nbsp;<a class="bot-link" href="http://138.2.12.110:8083/daytrade" >대시보드 ↗</a></span>
    </div>
  ...
  <div class="bot-card">
    <div class="bot-header">
      ...
      <span><button class="refresh-btn" onclick="loadCoinDay()">새로고침</button>&nbsp;<a class="bot-link" href="http://138.2.28.57:8081/daytrade" >대시보드 ↗</a></span>
    </div>
  ...
  <div class="bot-card">
    <div class="bot-header">
      ...
      <span><button class="refresh-btn" onclick="loadBear()">새로고침</button>&nbsp;<a class="bot-link" href="http://138.2.12.110:8083/bear" >대시보드 ↗</a></span>
    </div>
    <div id="bear-content"><p style="color:#64748b;font-size:12px;padding:10px">로딩 중...</p></div>
  </div>
  <div class="bot-card">
    <div class="bot-header">
      <span><span class="status-dot dot-loading" id="dot-gapup"></span><span class="bot-title">🚀 갭상승 단타봇 (주식)</span></span>
      <span><button class="refresh-btn" onclick="loadGapup()">새로고침</button>&nbsp;<a class="bot-link" href="http://138.2.12.110:8083/gapup" >대시보드 ↗</a></span>
    </div>
    <div id="gapup-content"><p style="color:#64748b;font-size:12px;padding:10px">로딩 중...</p></div>
  </div>
  <div class="bot-card">
    <div class="bot-header">
      <span><span class="status-dot dot-error" id="dot-prob"></span><span class="bot-title">🎲 코인 확률봇 v2</span> <span style="background:#7f1d1d;color:#fca5a5;padding:2px 8px;border-radius:4px;font-size:11px;font-weight:600">중지됨</span></span>
      <span></span>
    </div>
    <div id="prob-content"><p style="color:#94a3b8;font-size:13px;padding:16px;text-align:center">📋 4/3 폐지 — 단타봇에 통합<br><small style="color:#64748b">v1: 22건 -15,662원 | v2: 3건 -2,568원<br>교훈: CS≥80, 블랙리스트, 시간대 필터 → 단타봇 v4.0에 적용</small></p></div>
  </div>
  </div>
</div>
<script>
...
async function loadSwing() {
  setDot("swing","loading");
  try {
    const d = await fetch("http://138.2.12.110:8080/api/swing").then(r=>r.json());
    setDot("swing","ok");
    const posHTML = d.position
      ? `<div class="section-title" style="margin-top:8px">보유 포지션</div><table><tr><th>종목</th><th>매입가</th><th>수량</th></tr><tr><td><b>${d.position.name}</b></td><td>${fmt(d.position.buy_price)}</td><td>${d.position.qty}주</td></tr></table>`
      : `<p class="positions-empty">포지션 없음</p>`;
    document.getElementById("swing-content").innerHTML = `
      <div class="cards">
        <div class="card"><div class="lbl">자본</div><div class="val neu" style="font-size:14px">${fmt(d.capital)}원</div></div>
        <div class="card"><div class="lbl">수익률</div><div class="val ${pc(d.pnl_pct)}">${d.pnl_pct>=0?"+":""}${d.pnl_pct}%</div></div>
        <div class="card"><div class="lbl">누적 PnL</div><div class="val ${pc(d.total_pnl)}">${d.total_pnl>=0?"+":""}${fmt(d.total_pnl)}원</div></div>
        <div class="card"><div class="lbl">거래/승률</div><div class="val wht" style="font-size:14px">${d.trade_count}건 ${d.win_rate}%</div></div>
      </div>
      ${posHTML}`;
  } catch(e) {
    setDot("swing","error");
    document.getElementById("swing-content").innerHTML=`<p class="err-msg">❌ 연결 실패<br><small style="color:#64748b">${e.message}</small></p>`;
  }
}
...
async function loadCoinDay() {
  ...
  try {
    ...
    document.getElementById("coinday-content").innerHTML = `
      <div class="cards">
        <div class="card"><div class="lbl">전체 수익률</div><div class="val ${pc(d.total_return_pct||0)}">${(d.total_return_pct||0)>=0?"+":""}${d.total_return_pct||0}%</div></div>
        <div class="card"><div class="lbl">누적 손익</div><div class="val ${pc(d.total_pnl)}">${d.total_pnl>=0?"+":""}${fmt(d.total_pnl)}원</div></div>
        <div class="card"><div class="lbl">오늘 손익</div><div class="val ${pc(d.today_pnl)}">${d.today_pnl>=0?"+":""}${fmt(d.today_pnl)}원</div></div>
        <div class="card"><div class="lbl">오늘 거래/승률</div><div class="val wht" style="font-size:14px">${d.today_count}건 ${todayWR}%</div></div>
        <div class="card"><div class="lbl">전체 거래/승률</div><div class="val wht" style="font-size:14px">${d.trade_count}건 ${d.win_rate}%</div></div>
...
async function loadGapup() {
  setDot("gapup","loading");
  try {
    const d = await fetch("http://138.2.12.110:8083/api/gapup").then(r=>r.json());
    setDot("gapup","ok");
    const pos = d.positions || {};
    const posKeys = Object.keys(pos);
    const todayWR = d.today_count ? Math.round(d.today_wins/d.today_count*100) : 0;
    let posHTML = posKeys.length
      ? `<table><tr><th>종목</th><th>진입가</th><th>갭%</th><th>TV배수</th><th>TP</th><th>SL</th></tr>${posKeys.map(code=>{const p=pos[code];return `<tr><td><b>${p.name||code}</b></td><td>${fmt(p.entry_price)}���</td><td class="pos">+${(p.gap_pct||0).toFixed(1)}%</td><td class="neu">${(p.tv_mult||0).toFixed(1)}x</td><td class="pos">${fmt(p.tp_price)}원</td><td class="neg">${fmt(p.sl_price)}원</td></tr>`;}).join("")}</table>`
      : `<p class="positions-empty">포지션 없음</p>`;
    document.getElementById("gapup-content").innerHTML = `
      <div class="cards">
        <div class="card"><div class="lbl">누적 손익</div><div class="val ${pc(d.total_pnl)}">${d.total_pnl>=0?"+":""}${fmt(d.total_pnl)}원</div></div>
        <div class="card"><div class="lbl">오늘 손익</div><div class="val ${pc(d.today_pnl)}">${d.today_pnl>=0?"+":""}${fmt(d.today_pnl)}원</div></div>
        <div class="card"><div class="lbl">오늘 거래/승률</div><div class="val wht" style="font-size:14px">${d.today_count}건 ${todayWR}%</div></div>
        <div class="card"><div class="lbl">전체 거래/승률</div><div class="val wht" style="font-size:14px">${d.trade_count}건 ${d.win_rate}%</div></div>
        <div class="card"><div class="lbl">오픈 포지션</div><div class="val wht">${posKeys.length}/3</div></div>
      </div>
      <div class="section-title">보유 포지션</div>${posHTML}
      <div class="updated">봇 갱신: ${d.updated}</div>`;
  } catch(e) {
    setDot("gapup","error");
    document.getElementById("gapup-content").innerHTML=`<p class="err-msg">❌ 봇 미실행<br><small style="color:#64748b">장중에 daytrade_gapup.py 실행 필요</small></p>`;
  }
}
...
async function loadProb() {
  setDot("prob","loading");
  try {
    let r = await fetch("http://138.2.28.57:8081/api/prob");
    let d = await r.json();
    setDot("prob","ok");
    let pos = d.positions || {};
    ...
    let posHTML = "";
    if (posKeys.length > 0) {
      posHTML = posKeys.map(k => {
        let p = pos[k];
        let ret = ((p.trail_high - p.entry_price)/p.entry_price*100).toFixed(2);
        return '<div class="holding"><b>'+p.name+'</b> 스코어:'+(p.score*100).toFixed(1)+'% 진입:'+Math.round(p.entry_price).toLocaleString()+' 현재고점:'+Math.round(p.trail_high).toLocaleString()+' ('+ret+'%)<br><small>'+p.top_features+'</small></div>';
      }).join("");
    } else { posHTML = '<div class="holding" style="color:#64748b">포지션 없음</div>'; }
    let wr = d.daily.trades > 0 ? Math.round(d.daily.wins/d.daily.trades*100) : 0;
    document.getElementById("prob-content").innerHTML = '<div class="summary-grid"><div class="summary-item"><span class="label">오늘</span><span class="value">'+d.daily.trades+'건 '+wr+'%</span></div><div class="summary-item"><span class="label">오늘 PnL</span><span class="value '+pc(d.daily.pnl)+'">'+fmt(d.daily.pnl)+'원</span></div><div class="summary-item"><span class="label">총 PnL</span><span class="value '+pc(d.total.pnl)+'">'+fmt(d.total.pnl)+'원</span></div><div class="summary-item"><span class="label">포지션</span><span class="value">'+posKeys.length+'/'+d.config.max_positions+'</span></div></div><div class="section-title">보유 포지션</div>'+posHTML+'<div class="updated">스코어≥'+(d.config.score_threshold*100)+'% TP:'+(d.config.tp_pct*100)+'% SL:'+(d.config.sl_pct*100)+'% · '+d.config.coins+'코인</div>';
  } catch(e) {
    setDot("prob","error");
    document.getElementById("prob-content").innerHTML='<p class="err-msg">❌ 연결 실패<br><small style="color:#64748b">'+e.message+'</small></p>';
  }
}
function loadAll() {
  document.getElementById("global-updated").textContent = "마지막 갱신: "+new Date().toLocaleString("ko-KR")+" · 30초 자동갱신";
  loadCoin(); loadTesta(); loadEvo(); loadSwing(); loadDaytrade(); loadCoinDay(); loadBear(); loadGapup();
}
loadAll();
...
def gapup():
    ...
    return render_template_string(GAPUP_HTML, g=type('G', (), g)(), gc=gc)
...
GAPUP_HTML = """<!DOCTYPE html>
<html lang="ko">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<meta http-equiv="refresh" content="60">
<title>🚀 갭상승 단타봇 대시보드</title>
<script src="https://cdn.jsdelivr.net/npm/chart.js"></script>
<style>
* { box-sizing: border-box; margin: 0; padding: 0; }
body { font-family: 'Segoe UI', sans-serif; background: #0f1117; color: #e0e0e0; padding: 20px; }
h1 { font-size: 1.4rem; color: #e06cf0; margin-bottom: 6px; }
...
.cl-item { border-left: 3px solid #e06cf0; padding: 8px 14px; margin-bottom: 10px; background: #13161f; border-radius: 0 6px 6px 0; }
.cl-item .cl-date { font-size: 0.72rem; color: #666; }
.cl-item .cl-ver { font-size: 0.72rem; color: #e06cf0; margin-left: 8px; }
.cl-item .cl-title { font-size: 0.85rem; color: #e0e0e0; margin-top: 3px; }
.cl-item .cl-detail { font-size: 0.75rem; color: #888; margin-top: 2px; }
</style>
</head>
<body>
<h1>🚀 갭상승 단타봇 <span style="font-size:13px;color:#94a3b8">갭업 2~15% + 거래대금 2배</span>
  <a class="back" href="/hub">← 허브로</a>
</h1>
<div class="sub">봇 업데이트: {{ g.updated }} · 60초 자동갱신</div>

<div class="cards">
  <div class="card"><div class="lbl">누적 손익</div><div class="val {{ 'pos' if g.total_pnl >= 0 else 'neg' }}">{{ "{:+,}".format(g.total_pnl) }}원</div></div>
  <div class="card"><div class="lbl">오늘 손익</div><div class="val {{ 'pos' if g.today_pnl >= 0 else 'neg' }}">{{ "{:+,}".format(g.today_pnl|int) }}원</div></div>
  <div class="card"><div class="lbl">오늘 거래</div><div class="val wht">{{ g.today_count }}건 / 승 {{ g.today_wins }}건</div></div>
  <div class="card"><div class="lbl">전체 거래수</div><div class="val wht">{{ g.trade_count }}건</div></div>
  <div class="card"><div class="lbl">전체 승률</div><div class="val {{ 'pos' if g.win_rate >= 50 else 'neg' }}">{{ g.win_rate }}%</div></div>
  <div class="card"><div class="lbl">R/R</div><div class="val {{ 'pos' if g.rr >= 1.5 else 'neg' }}">{{ g.rr }}</div></div>
  <div class="card"><div class="lbl">평균 수익/손실</div><div class="val wht" style="font-size:14px">{{ "{:+.2f}".format(g.avg_win) }}% / {{ "{:.2f}".format(g.avg_loss) }}%</div></div>
  <div class="card"><div class="lbl">오픈 포지션</div><div class="val wht">{{ g.positions|length }} / 3</div></div>
</div>
...
<div class="section">
  ...
  {% if g.positions %}
  <table>
    <tr><th>종목</th><th>진입시각</th><th>진입가</th><th>수량</th><th>갭%</th><th>TV배수</th><th>TP</th><th>SL</th></tr>
    ...
    <tr>
      ...
      <td>{{ "{:,}".format(pos.entry_price) }}원</td>
      <td>{{ pos.qty }}주</td>
      <td class="pos">+{{ "{:.1f}".format(pos.gap_pct) }}%</td>
      <td class="neu">{{ "{:.1f}".format(pos.tv_mult) }}x</td>
...
<!-- 오늘 거래 -->
<div class="section">
  <div class="section-title">오늘 거래</div>
  {% if g.today_closed %}
  <table>
    ...
    <tr>
      ...
      <td><span class="badge {{ 'b-pos' if (tr.return_pct or 0) >= 0 else 'b-neg' }}">{{ "{:+.2f}".format(tr.return_pct or 0) }}%</span></td>
      <td><span class="badge {{ 'b-pos' if tr.result == 'TP' else ('b-neg' if tr.result == 'SL' else 'b-neu') }}">{{ tr.result }}</span></td>
      <td class="pos">+{{ "{:.1f}".format(tr.gap_pct or 0) }}%</td>
...
<!-- 최근 거래 -->
<div class="section">
  <div class="section-title">최근 거래 (최대 20건)</div>
  {% if g.recent_trades %}
  <table>
...
<div class="section">
  ...
  {% endfor %}
  {% else %}
  <p style="color:#555;font-size:0.83rem;">기록 없음</p>
  {% endif %}
</div>
