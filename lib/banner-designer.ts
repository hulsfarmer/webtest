...
- ⚠️ **헤드라인↔보조문구 간격을 크게(붙지 않게)**: 보조문구 baseline의 y = (헤드라인 **마지막 줄** baseline의 y) + (헤드라인 font-size × **1.1 이상**). 즉 헤드라인 한 줄 높이만큼 통째로 띄워 뚜렷한 빈 공간을 둬. 둘이 닿거나 겹쳐 보이면 실패. 라벨↔헤드라인 사이도 헤드라인 font-size의 0.5배 이상 여유.
- 글자 겹침/삐져나감 금지. 한글 길이에 맞춰 font-size 조절(작게: 한글 8자↓ ~82px, 14자↓ ~62px, 20자↓ ~50px). 위 "작게·여백" 규칙이 폰트 크기보다 우선 — 애매하면 더 작게.
